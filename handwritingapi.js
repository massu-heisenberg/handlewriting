...
QQShuru.Util.Event.getPoint = function (a) {
    if (QQShuru.Util.Browser.isIE) {
        ...
        const touch = a['touches'] && a['touches'][0];
        const left = a.layerX || Math.floor(touch['pageX'] - vertex.left);
        const right = a.layerY || Math.floor(touch['pageY'] - vertex.top);
        return [left, right];
    }
};

QQShuru.Util.Event.getVertexPosition = function (el) {
...
QQShuru.HWPanel = function (obj) {
    ...
    var K = [];
    var O = [];
    var ver = QQShuru.Util.Event.getVertexPosition;
